...
command! BEvalDemoStart
\   call s:start()

command! BEvalDemoStop
\   call s:stop()
...
function! s:BufferCommon.setup_common()
    ...
    silent file ___BEvalDemo___
endfunction

function! s:BufferCommon.finalize()
    if input('Do you sure want to force-stop? (use '':BEvalDemoStop'' for normal stop) [y/n]: ') !~? 'y\%[es]'
        echo "\nCanceled."
...
function! s:BufferPlaying.setup()
    ...
    " Add highlight.
    execute 'syn match BEvalDemoRed /'.s:CURSOR_RED.'/'
    highlight BEvalDemoRed term=reverse cterm=bold ctermfg=1 ctermbg=1 guifg=Red guibg=Red
    " FIXME: Do not change default highlight!
    " highlight def link BEvalDemoCursor Cursor
    " highlight BEvalDemoCursor term=reverse cterm=bold ctermfg=1 ctermbg=1 guifg=Red guibg=Red
